...
//app.use(enforceHttps({ trustProtoHeader: true})); // for reverse proxies
...
if (env !== 'test') {
    ...
    if(env === 'production') {
        /* TODO: not working for https, so comment out it for now
        if(cluster.isMaster) {
...
if(config.get('is_using_ssl')){
    const ssl_port = process.env.SSLPORT ? parseInt(process.env.SSLPORT) : 8443;
    const sslOptions = {
        key: fs.readFileSync(path.join(__dirname, './ssl/weku_io.key')),
        cert: fs.readFileSync(path.join(__dirname, './ssl/weku_io.crt'))
    };
    https.createServer(sslOptions, app.callback()).listen(ssl_port);
}
