...
//import {getCommunityLanguage} from "app/utils/CommunityUtil";
...
function* confirmEmailHandler() {
    ...
    if (eid.email_verified) {
        ...
        // Skip approval step, direct go to create_account page
    ...
    // TODO: need to add logic to ignore email uniqueness check during testing
    ...
    // since we skip the manual approve step, so change the 'waiting' directly to 'approved' status here.
    //yield models.User.update({ account_status: 'waiting'}, {
    //yield models.User.update({ account_status: 'approved'}, {
    //    where: { id: eid.user_id, account_status: 'onhold' }
    //});
    if (mixpanel)
        mixpanel.track("SignupStepConfirmEmail", { distinct_id: this.session.uid });

    if(config.get('phoneValidation')){ // if need mobile phone validation, then go phone enter page
    ...
    }else{ // no need to verify phone, go to create account page dirrectly
...
export default function useEnterAndConfirmEmailPages(app) {
    ...
    // step 1: display form to collect email address
    router.get("/enter_email", function*() {
        ...
        tt.setLocale('en');
        if (this.request.header.host === "news.weku.io") {
		    tt.setLocale('zh');
        }
        ...
        const referralDefaultValue = this.request.query.referral;

        // TODO: need to use multi-language support to replace hardcode Chinese
                ...
                <div className="row" style={{ maxWidth: "32rem" }}>
                    <div className="column">
                        ...
                        <form id="submit_email" action="/submit_email" method="POST">
                            ...
                            <p className="secondary">{tt('g.warning_email')}</p>
                            <input
                                type="hidden"
                                name="csrf"
                                value={this.csrf}
                            ...
                            <label>
                                ...
                                <input
                                    type="text"
                                    name="referral"
                                    defaultValue={referralDefaultValue}
                                    readOnly
                                />
                            </label>
                            <br />
    ...
    // Step 2: after user entered email, and click 'continue' button, this js will run.
    ...
    // step 3: process after user filled form and click 'continue' button
    // during this step, start to create record in db table: users
    router.post("/submit_email", koaBody, function*() {
        if (!checkCSRF(this, this.request.body.csrf)) return;

        let {email, account, referral} = this.request.body;
        ...
        try {
            ...
            // create a record into table: users
            // uid is a seperate column in table: users
            // id column in table:users is auto-incremented.
            // key columns here:
            // id, name(=account), email, uid(=session id)
            // verified: if email / phone is verified.
            // account status: onhold -> waiting -> approved -> created
            // onhold: just registered, email/phone is not verified yet.
            // after verified emai/phone, it should go to waiting,
            // but since we skip the manul approve step, it will go directly to approved.
            const user = yield models.User.create({
                uid: this.session.uid,
                remote_ip: getRemoteIp(this.request.req),
                sign_up_meta: JSON.stringify({last_step: 2}),
                // hardcode here, skip manual approve step
                account_status: 'onhold',
            ...
            //?? not sure what's the purpose of adding a referer record into user_attributes table here.
            ...
            // already record referral in table:users,
            // no need to add another same record in table:user_attributes
            /*
            if(referral) {
            ...
            // create a record into table:identities,
            // which has a many-to-one (normally 2-1) relationship with table:users
            ...
            sendEmail("confirm_email", email, {hostname: this.hostname, confirmcode: confirmation_code });
        ...
        //this.redirect("/enter_mobile");
        // need to add a page to show user to verify their email before continue to next step.
        this.flash = { success: "Thanks. Your email needs to be confirmed before proceeding." };

        this.redirect("/");
        //this.redirect("/before_email_confirmed");
    });

    // step 4: show flash info for user to confirm their email address in mailbox

    // step 5: handle confirm email
    // this route will be hit after users click the link in their mailbox.
    router.get("/confirm_email/:code", confirmEmailHandler);

    // step 5: handle confirm email in different way
    router.post("/confirm_email", koaBody, confirmEmailHandler);

    // ?? not sure when this route get hit.
    // should be step: 6 if manual approve step is enabled.
    // maybe used for skipping step: manual approve.
    // after manual approve, user should receive a email include the link /start:code
    // then user click the link in their mailboxes, then visit this route.
    // since we skipped the manual approve step, this is not used for now.
