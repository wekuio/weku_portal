...
export default function useGeneralApi(app) {
    ...
    router.get('/categories/:community', koaBody, function *() {
        const community = this.params.community;
        console.log('-- GET /categories/:community -->', this.session.uid, community);
        try{
            const categories = yield models.Category.findAll({where: { community}});
            this.body = JSON.stringify({status: 'ok', categories});
        }catch (error) {
    ...
    router.get('/is_in_blacklist/:account', koaBody, function *(){
        console.log('is_in_blacklist begin');
        const account_name = this.params.account;
        try{
            const blacklist = yield models.Blacklist.findOne({where: { account_name}});
            this.body = JSON.stringify({status: blacklist ? true : false});
        }catch (error) {
            console.error('Error in get /is_in_blacklist', error);
            this.body = JSON.stringify({error: error.message});
            this.status = 500;
        }
    });

    router.get('/is_bookmarked', koaBody, function *(){
        const{account, post_url, community} = this.query;
        const account_name = account;
        console.log(`is_bookmarked begin: account_name:${account_name}, post_url:${post_url} community:${community}`);
        try{
            const bookmark = yield models.Bookmark.findOne({where: { account_name: account, post_url, community}});
            this.body = JSON.stringify({status: bookmark ? true : false});
        }catch (error) {
            console.error('Error in get /is_bookmarked', error);
            this.body = JSON.stringify({error: error.message});
            this.status = 500;
        }
    });

    router.post('/create_bookmark', koaBody, function *(){
        const params = this.request.body;
        const {account, post_url, community, post_data, csrf} = typeof(params) === 'string' ? JSON.parse(params) : params;
        const account_name = account;
        if (!checkCSRF(this, csrf)) return;
        console.log(`create_bookmark begin: account_name:${account_name}, post_url:${post_url} community:${community}`);

        try{
            const bookmark = yield models.Bookmark.findOne({where: {account_name, post_url, community}});
            if(!bookmark)
                 yield models.Bookmark.create({ account_name, post_url, community, post_data});

            this.body = JSON.stringify({status: 'ok'});

        }catch (error) {
            console.error('Error in post /create_bookmark', error);
            this.body = JSON.stringify({error: error.message});
            this.status = 500;
        }
    });

    router.post('/remove_bookmark', koaBody, function *(){
        const params = this.request.body;
        const {account, post_url, community, csrf} = typeof(params) === 'string' ? JSON.parse(params) : params;
        const account_name = account;
        if (!checkCSRF(this, csrf)) return;
        console.log(`remove_bookmark begin: account_name:${account_name}, post_url:${post_url} community:${community}`);

        try{
            yield models.Bookmark.destroy({where: {account_name, post_url, community}});

            this.body = JSON.stringify({status: 'ok'});

        }catch (error) {
            console.error('Error in post /remove_bookmark', error);
            this.body = JSON.stringify({error: error.message});
            this.status = 500;
        }
    });

    router.get('/get_bookmarks/:account/:community', koaBody, function *(){
        const{account, community} = this.params;
        const account_name = account;
        console.log(`get_bookmarks> account_name:${account_name}, community:${community}`);
        try{
            const bookmarks = yield models.Bookmark.findAll({where: { account_name, community}});
            this.body = JSON.stringify({status: 'ok', bookmarks});
        }catch (error) {
            console.error('Error in get /get_bookmarks', error);
            this.body = JSON.stringify({error: error.message});
            this.status = 500;
        }
    });
    ...
    router.post('/accounts', koaBody, function *() {
        console.log('--- enter accounts route ---');
        if (rateLimitReq(this, this.req)) return;
        const params = this.request.body;
        const account = typeof(params) === 'string' ? JSON.parse(params) : params;
        if (!checkCSRF(this, account.csrf)) return;
        ...
        try {
            ...
            //     throw new Error("Only one WeKu account per user is allowed in order to prevent abuse");
            // }
            ...
            if (same_ip_account) {
                ...
                if (minutes < 10) {
                    ...
                    throw new Error('Only one WeKu account allowed per IP address every 10 minutes');
                }
            }
