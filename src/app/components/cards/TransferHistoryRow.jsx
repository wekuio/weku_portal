    ...
    render() {
        ...
        if( type === 'transfer_to_vesting' ) {
            if( data.from === context ) {
                if( data.to === "" ) {
                    description_start += tt('g.transfer') + data.amount.split(' ')[0] + tt('g.to') + "WEKU POWER";
                }
                else {
                    description_start += tt('g.transfer') + data.amount.split(' ')[0] + " WEKU POWER" + tt('g.to');
                    other_account = data.to;
                }
            }
            else if( data.to === context ) {
                description_start += tt('g.receive') + data.amount.split(' ')[0] + " WEKU POWER" + tt('g.from');
                other_account = data.from;
            } else {
                description_start += tt('g.transfer') + data.amount.split(' ')[0] + " WEKU POWER" + tt('g.from') + data.from + tt('g.to');
                other_account = data.to;
            }
        }
            ...
            if( data.vesting_shares === '0.000000 VESTS' )
                ...
                description_start += tt('transferhistoryrow_jsx.start_power_down_of') + ' ' + powerdown_vests + " WEKU";
        } else if( type === 'curation_reward' ) {
            description_start += `${curation_reward} WEKU POWER` + tt('g.for');
            other_account = data.comment_author + "/" + data.comment_permlink;
        } else if (type === 'author_reward') {
            let steem_payout = "";
            if(data.steem_payout !== '0.000 WEKU') steem_payout = ", " + data.steem_payout;
            description_start += `${data.sbd_payout}${steem_payout}, ${tt('g.and')} ${author_reward} WEKU POWER ${tt('g.for')} ${data.author}/${data.permlink}`;
            // other_account = ``;
            description_end = '';
            ...
            if(parseFloat(data.reward_vests.split(' ')[0]) > 0) rewards.push(`${reward_vests} WEKU POWER`);
          ...
          let steem_payout = "";
          if(data.steem_payout !== '0.000 WEKU') steem_payout = ", " + data.steem_payout;
          description_start += `${benefactor_reward} WEKU POWER for ${data.author}/${data.permlink}`;
          description_end = '';
        } else {
