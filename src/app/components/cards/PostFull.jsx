...
import {serverApiRecordEvent, getIsBookmarked, createBookmark} from 'app/utils/ServerApiClient';
...
class PostFull extends React.Component {
    ...
    componentDidMount(){
        const post_url = this.getPostUrl();
        if (this.props.username && post_url){
            getIsBookmarked(this.props.username, post_url, this.state.community).then((isBookmarked) => {
                if(isBookmarked) this.setState({isBookmarked});
            });
        }
    }
    ...
    onBookmark = () => {
        const post_url = this.getPostUrl();
        const post_content = this.props.cont.get(this.props.post);
        if(!this.props.username || !post_url || !post_content) return;

        const content = JSON.stringify(post_content.toJS());
        createBookmark(this.props.username, post_url, this.state.community, content);
    ...
    render() {
        ...
        this.share_params = {
            link,
            url: 'https://deals.' + APP_DOMAIN + link,
        ...
        const showBookmark = (this.props.username != undefined) && !this.state.isBookmarked;
        const showBookmarked = (this.props.username != undefined) && this.state.isBookmarked;

        return (
          <article className="PostFull hentry" itemScope itemType="http://schema.org/blogPost">
              <div className="">
                  {showBookmark &&
                  <a onClick={this.onBookmark}>Bookmark</a>
                  }
                  {showBookmarked &&
