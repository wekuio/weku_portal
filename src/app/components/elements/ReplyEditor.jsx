...
export default formId => connect(
    ...
    dispatch => ({
        ...
        reply: ({category, title, body, author, permlink, parent_author, parent_permlink, isHtml, isStory,
            ...
            if(community && category.indexOf(community) < 0)
                category = community + ' ' + category; // put community as first tag? last tag? to be or not to be?
                ...
                switch(payoutType) {
                    case '0%': // decline payout
                        __config.comment_options = {
                            max_accepted_payout: '0.000 WKD',
                        }
                        break;
                    case '100%': // 100% weku power payout
                        __config.comment_options = {
                            percent_steem_dollars: 0, // 10000 === 100% (of 50%)
                        }
                        break;
                    default: // 50% weku power, 50% wkd+weku
                }
            }
