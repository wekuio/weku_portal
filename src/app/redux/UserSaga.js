...
import {serverApiLogin, serverApiLogout, getIsInBlacklist} from 'app/utils/ServerApiClient';
...
function* usernamePasswordLogin2({payload: {username, password, saveLogin,
    ...
    if (username && DMCAUserList.includes(username)) {
        yield put(user.actions.loginError({ error: 'account disabled, due to terms violation.' }))
        return;
    }
    const isInBlacklist = yield getIsInBlacklist(username);
    if(isInBlacklist){
        yield put(user.actions.loginError({ error: 'account disabled, due to terms violation.' }))
        return;
    }
    ...
    if (!fullAuths.size) {
        ...
        if (owner_pub_key === 'STM7sw22HqsXbz7D2CmJfmMwt9rimtk518dRzsR1f8Cgw52dQR1pR') {
            yield put(user.actions.loginError({ error: 'Hello. Your account may have been compromised. We are working on restoring an access to your account. Please send an email to support@weku.io.' }))
            return
        }
