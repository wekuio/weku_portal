            ...
            reducer: (state, {payload: {operations, error, errorCallback}}) => {
                ...
                for (const [type/*, operation*/] of operations) {
                    ...
                    case 'withdraw_vesting':
                        if(/Account registered by another account requires 10x account creation fee worth of Weku Power/.test(errorStr))
                            errorKey = 'Account requires 10x the account creation fee in WEKU Power (approximately 300 WP) before it can power down.'
                        break;
                    default:
                        break;
                    }
                    if (state.hasIn(['TransactionError', type + '_listener'])) {
                        ...
                        if (/unknown key: /.test(errorKey)) {
                            errorKey = "WEKU account doesn't exist.";
                            errorStr = "Transaction failed: WEKU account doesn't exist.";
                        }
