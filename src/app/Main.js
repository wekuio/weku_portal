...
import {getCommunityLanguage} from "./utils/CommunityUtil";
...
function runApp(initial_state) {
    ...
    initial_state.user.locale = getCommunityLanguage(host);
    
    const location = `${window.location.pathname}${window.location.search}${window.location.hash}`;
    universalRender({history, host, location, initial_state})
    .catch(error => {
        console.error(error);
